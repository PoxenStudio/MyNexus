...
> **已知局限**：混合检索的关键词侧（BM25）目前仍是"每次查询时加载全部候选分块、用 `rank_bm25` 现场建索引"，这个限制与向量库选型无关，在 `book_ids` 未指定（全库检索）且书籍规模很大时会成为新的瓶颈。后续如需解决，可考虑把关键词检索能力下沉到 Core API 侧的 SQLite FTS5 全文索引（`chunks` 表已在 Core API 侧持久化），而不是在 Worker 侧重复维护一份全量文本副本。
