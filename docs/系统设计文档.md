...
同样地，切片阶段的 "token 数" 用**字符数近似**（`nodes/splitters/token_splitter.py`），未引入 `tiktoken` 等分词器——这一项目前仍是已知取舍，见 `docs/Todos.md`。

混合检索的关键词侧原先是 `rank_bm25` + 一个字符级 CJK 分词正则（每个中文字符视为一个 token），2026-08-05 起改为 `jieba`（`pipelines/retrieval.py` 的 `_tokenize`）——按词切分而不是按字，BM25 的词频/逆文档频率统计能反映真实词语而不是单字，对中文关键词检索的相关性有直接提升。`jieba.initialize()` 在模块导入时就调用，把首次建词典树（约 1s）的开销放在 Worker 启动阶段，不拖慢第一次真实检索请求；这个改动只影响 SQLite 后端的现场建索引路径，Postgres 后端走的是 `ts_rank`/GIN，不受影响。
