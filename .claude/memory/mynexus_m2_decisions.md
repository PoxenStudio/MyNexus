...
**Core API owns all DB writes; Worker is stateless and reports via callback (HTTP originally, gRPC since 2026-08-05 — see [[mynexus_grpc_migration]]).**
docs/系统设计文档.md §2.3's sequence diagram literally shows `SPLITTER -- "Chunk 元数据回写" --> RDBMS` (Worker writing directly to the shared SQLite file). That was deliberately not implemented that way — instead Worker reports progress/complete/fail back to Core API (`ReportProgress`/`ReportComplete`/`ReportFail` gRPC calls; originally HTTP POSTs to `/internal/tasks/{id}/{progress,complete,fail}`), and only Core API touches the database.
Why: avoids duplicating the schema/migration logic in two languages, avoids multi-process SQLite write contention on NAS hardware, keeps DB ownership in one place matching §1.2's "Core API 负责元数据 CRUD" framing. This reasoning was explicitly re-examined (not just assumed) when Postgres was added as a backend option ([[mynexus_postgres_backend]]) and again during the keyword-search GIN work ([[mynexus_keyword_search_gin]]) — Postgres's better concurrent-write handling removes the *first* reason but not the second (schema-duplication risk is independent of which database is used), so the callback pattern stayed even though a technically-safer alternative (Worker writing directly) became available.
How to apply: keep this pattern for any future Worker->Core API data flow. If a future contributor points at §2.3's diagram literally, flag this documented divergence rather than "fixing" it back to direct writes. The transport carrying the callback (HTTP vs gRPC) is a separate, orthogonal decision from whether Worker writes directly — don't conflate "we changed the transport" with "we changed who owns writes."
