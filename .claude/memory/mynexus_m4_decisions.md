...
**API Token's "last 4 characters shown" required a schema change.** The design only ever stores `token_hash` (SHA-256), so there's no way to recover the real last-4 of the raw token from the hash for display. Added a `token_suffix` column to the `api_tokens` table (captured at creation time, before the raw token is discarded) — see `migrations/0001_init.sql`. Edited the original migration file directly rather than adding a new numbered one, since `sqlite.Store.migrate()` re-runs every `.sql` file's *full contents* on every startup with no applied-migrations tracking (fine for `CREATE TABLE IF NOT EXISTS`, but an `ALTER TABLE ADD COLUMN` would fail on the second run) — and no real deployments exist yet to have data worth preserving through a proper migration.
