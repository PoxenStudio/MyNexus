...
4. **No login flow exists**, so `middleware.APITokenAuth` is enforced-when-present rather than mandatory: requests with no `Authorization` header pass through as `defaultUserID = "local-user"` on every route. This means, as shipped, the admin API is effectively open to anyone who can reach it unless they deliberately configure and require a token out-of-band — there is no way to *lock out* unauthenticated access today. See [[mynexus_m4_decisions]].
...
- **`tasks.stages_log` column is written but never populated with real content** — every task is created with `stages_log: "[]"` and nothing ever updates it. The Tasks admin page only surfaces `error_msg`, not structured per-stage progress/log entries, despite the schema being ready for it.
- **No bulk book operations** (multi-select delete/rebuild) — 需求文档.md §6.7.3 asks for this, web-ui only supports single-book actions.
- **No admin action audit log** — 需求文档.md §6.7.3's "操作审计记录" is unbuilt; no table or logging exists for who did what admin action when.
- **No end-user chat/QA page** — `worker`'s QA pipeline and Core API's `/chat` endpoints are fully implemented and tested via curl/mock server, but `web-ui` only has the admin backend; 需求文档.md §6.7.1's end-user-facing chat UI was never in any milestone's deliverable list and remains entirely unbuilt.
