...
Known remaining gap: hybrid search's BM25/keyword side still loads the full candidate set and rebuilds a `rank_bm25` index per query (see §3.4's "已知局限") — this is independent of the vector store choice and would need Core API-side SQLite FTS5 (or similar) to scale to a very large whole-library search.
