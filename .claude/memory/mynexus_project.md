...
- `worker/` (renamed from `worker-ai` per user request) — Python, stateless capability node layer (parse/clean/split/embed/retrieve/LLM). Structured as `src/{nodes/{parsers,cleaners,splitters,embedders,llm},pipelines,vector_store,schemas}`. Network entrypoint (`server.py`) is a **gRPC** server, not FastAPI/HTTP — see [[mynexus_grpc_migration]].
